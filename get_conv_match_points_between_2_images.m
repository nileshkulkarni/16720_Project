function matched_points = get_conv_match_points_between_2_images(net, image1, cp1,...
    image2, cp2, points3D, pixel_threshold, downscale)
    
    newSize = size(image1)*downscale;
    newSize = floor(newSize(1:2));
    ...
    features1 = getCNNFeatures(net, image1);
    size_f = size(image1);
    indices = sub2ind(size_f(1:2), keypointsImage1(:,2), keypointsImage1(:,1));
    ...
    allFeaturesImage2 = getCNNFeatures(net, image2);
    ...
    for j=1:size(features1,1)
        ...
        minDist = Inf;
        mini = nan;
        difference = sum((allFeaturesImage2 - currentImageFeature).^2, 2);
